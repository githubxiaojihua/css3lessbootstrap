...
​	
	5.css声明的优先级
		选择器的特殊性(通过特殊性来判断冲突样式的应用规则)
			选择器的特殊性由选择器本身的组件确定，这里将特殊性值表述为4个部分，如0,0,0,0
			一个选择器的具体特殊性如下确定：
				...
				实例：id选择器和属性选择器
				      div[id="test"]（0,0,1,1） 和 #test（0,1,0,0） ，当这两个选择器同时作用于一个元素时且样式有冲突时，
					  0100大于0011的特殊性，因此会应用0100的样式
		重要声明（!important来标注）
			...
			!important 总是要放在声明的最后，即分号的前面
			 ...
			 实际上所有的重要声明会被浏览器分为一组，冲突通过其特殊性来解决
			 非重要声明也会被分为一组，冲突通过其特殊性来解决
			 如果一个重要声明与非重要声明冲突，胜出的总是重要声明
		继承
			继承没有特殊性，继承过来的模式甚至连0特殊性都没有
			0特殊性（比如文档开始时候的*通配符描述的样式）要比无特殊性来的强
		来源
			css样式的来源大致有三种
			  创作人员
			  读者（只有IE浏览器提供了相关方式，其它的都没有提供，具体看下文图一）
			  用户代理   
...
![](images/微信截图_20200928192250.png)